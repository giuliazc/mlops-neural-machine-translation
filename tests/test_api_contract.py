...
def test_reload_accepts_empty_body():
    """POST /reload sem body deve aceitar o contrato e falhar apenas por falta de modelo."""
    client = _client()
...
def test_reload_accepts_optional_run_id_and_artifacts_dir():
    """POST /reload aceita run_id e artifacts_dir opcionais."""
    client = _client()
    r = client.post(
        "/reload",
        json={
    ...
    assert r.status_code == 500
    assert "SavedModel não encontrado" in r.json()["detail"]


def test_openapi_documents_expected_paths():
    """OpenAPI deve expor os endpoints de contrato da API."""
    client = _client()
