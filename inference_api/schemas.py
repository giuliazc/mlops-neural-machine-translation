...
class ReloadRequest(BaseModel):
    ...
    Attributes:
        ...
        artifacts_dir: Diretório alternativo de cache local. Se omitido, usa o configurado.
        artifact_bucket: Bucket S3 alternativo. Se omitido, usa ARTIFACT_BUCKET.
        artifact_prefix: Prefixo S3 alternativo. Se omitido, usa ARTIFACT_PREFIX.
    """
    ...
    artifacts_dir: str | None = Field(
        default=None,
        description="Diretório alternativo de cache local de artefatos (ex.: '/tmp/artifacts').",
    )
    artifact_bucket: str | None = Field(
        default=None,
        description="Bucket S3 alternativo para buscar artefatos publicados.",
    )
    artifact_prefix: str | None = Field(
        default=None,
        description="Prefixo S3 alternativo para buscar artefatos publicados.",
    )
    ...
    def __init__(self, **data):
        ...
        if self.artifact_prefix:
            self.artifact_prefix = self.artifact_prefix.strip('/')
